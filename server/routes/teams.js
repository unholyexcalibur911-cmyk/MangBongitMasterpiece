...
router.post('/', requireAuth, async (req, res) => {
  ...
  const team = await Team.create({
    ...
    description,
    settings,
    members: [{ userId: req.user.sub, email: req.user.email, role: 'owner', isActive: true }],
...
router.post('/:id/join', requireAuth, async (req, res) => {
  ...
  if (!team.members.some(m => m.userId === req.user.sub)) {
    team.members.push({ userId: req.user.sub, email: req.user.email, role: 'member', isActive: true });
